...
@if ($data)
    ...
    @if ($data->end_date < date('Y-m-d H:i:s') && $data->status == 1)
        ...
        bg-gray-100 dark:bg-gray-900 dark:text-gray-100 mx-2">
            <div class="flex items-center justify-between py-0 px-3">
                ...
                <div class="text-right">
                    ...
                    @if ($data->user->id == Auth::user()->id)
                        <div class="mt-0 pt-0">
                            <form action="meus-pedidos/{{ $data->id }}" method="POST" id="form_{{ $data->id }}" class=" container mt-0 pt-0 flex flex-col mx-auto space-y-4 ng-untouched ng-pristine ng-valid">
                                @csrf
                            </form>
                             <button type='button' onclick="del({{$data->id}})" class="bg-red-500
