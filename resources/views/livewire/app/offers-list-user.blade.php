                    ...
                    <div class="stat-actions">
                        @if ($item->demand->offer_id == $item->id)
                            @if ($item->demand->status >=2 && $item->status == 1)
                                <button wire:click="showModalUpdate({{ $item->id }})" wire:key="item-{{ $item->id }}" class="flex
                                    ...
                                    <svg class="w-5 h-5 mr-2" fill="currentColor" focusable="false" aria-hidden="true" viewBox="0 -64 640 640"
                            ...
                            @if ($item->demand->status >= 2)
                                ...
                                @else
                                    <span
                                        ...
                                        outline-none focus:outline-none lg:mr-1 lg:mb-0 ml-3 mb-4
                                        ease-linear transition-all duration-150">
                                            Aguardando {{ $item->order }}º na fila
                                    </span>
                                @endif
            ...
            <form action="#" wire:submit.prevent="update()" wire.loading.attr='disable'>
                <div class="grid gap-4 mb-1 sm:grid-cols-1 sm:gap-6 sm:mb-5">
                    <div class="sm:col-span-2">
                        ...
                        <select wire:model="account_id" name="account_id" id="account_id"
                            placeholder="Conta"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                            <option value="">Selecione uma opção</option>
                            @if ($accounts)
                                @foreach ($accounts as $account)
                                    <option value="{{ $account->id }}" >{{ $account->name }} ({{ $account->category->title }})</option>
                                @endforeach
                            @endif
                        </select>
